...
int sort(int index,int block_num,void *value1,void *value2){
  ...
  for(i=*counter-1;i>=0;i--){
    void * temp;
    temp = &data[m+(attrLength1+attrLength2+sizeof(int))*i];
    ...
    if(!compare(value1,temp,Open_Files[index]->attrType1,attrLength1)){ 
      memcpy(&data[m+(attrLength1+attrLength2+sizeof(int))*(i+1)],(char *)temp,attrLength1);
      temp = &data[m+(attrLength1+attrLength2+sizeof(int))*i+attrLength1];
      memcpy(&data[m+(attrLength1+attrLength2+sizeof(int))*(i+1)+attrLength1],(char *)temp,attrLength2);
    }
    //Else insert the new entry, right of the current
    else{
      memcpy(&data[m+(attrLength1+attrLength2+sizeof(int))*(i+1)],(char *)value1,attrLength1);
      memcpy(&data[m+(attrLength1+attrLength2+sizeof(int))*(i+1)+attrLength1],(char *)value2,attrLength2);
      flag = 1;
      break;
    }
  }
